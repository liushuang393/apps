# Sonowa スタック起動（Windows PowerShell・Cursor 外推奨）
...
Write-Host "[Sonowa] docker compose up -d --build"
docker compose up -d --build

Write-Host "[Sonowa] alembic upgrade head"
docker compose exec backend alembic upgrade head

Write-Host "[Sonowa] waiting for /health"
$ok = $false
for ($i = 0; $i -lt 60; $i++) {
  try {
