...
- 文書 ID: `sonowa-e2e-architecture-2026-09-06`
- 状態: 承認済み方針（案 B: kit 契約中心 + Sonowa 差分）
- 対象: Sonowa MVP + testing-kit v0.3.0（installed-kit）
- 作成日: 2026-09-06

## 1. 目的

多言語会議システム Sonowa を、**業務シナリオ駆動の E2E** で繰り返し検証し、失敗を分類・修正・再実行して収束させる。  
理想状態は次の一本道が通ること:
...
flowchart TB
  ...
  subgraph sonowa [SONOWA_project_owned]
...
| 層 | 持つもの | 持たないもの |
|----|----------|--------------|
| **kit 共通** | 手順、lane、doctor、レポート、fail-closed、契約検証 | Sonowa 業務意味 |
| **adapter** | FastAPI+React の起動／認証モード／能力宣言 | 会議室ドメイン |
| **Sonowa** | JWT persist、LiveKit、字幕、AI 設定シナリオ、testid | 独自 Playwright 正本の常設並存 |
...
stub_base_url_env = "SONOWA_AI_STUB_URL"
```
...
| 段階 | 手段 | 位置づけ |
|------|------|----------|
| 暫定 | `MockAIProvider`（`SONOWA_E2E_MOCK_AI=1` / `ai_provider=mock`） | A レーンを動かすための移行手段 |
...
| 方式 | 条件 | 備考 |
|------|------|------|
| **現行** | `seed_mode=self` + `e2e_*` プレフィックス | 共有 DB `sonowa` は wipe 禁止（URL に test/e2e/ci 無し） |
| **推奨将来** | DB 名 `sonowa_e2e` 等 | kit の reset fail-closed と整合 |
...
- フロント: zustand persist キー `sonowa-auth`
...
| testing-kit ツリーが root 所有で書込不可な場合 | installed-kit（wheel）＋ Sonowa 内契約で吸収 | kit 本体改修は書込可能時／別作業 |
...
- Sonowa: [`docs/テスト観点.md`](../テスト観点.md), [`CLAUDE.md`](../../CLAUDE.md), [`.cursor/rules/docker-windows-startup.mdc`](../../.cursor/rules/docker-windows-startup.mdc)
