...
本設計は、次の3プロジェクトをコードレベルで比較し、Sonowa-MVPのリアルタイム会議基盤を改善するものである。
...
| WebRTC／LiveKitを維持       | **採用継続**      | Sonowaの多人数会議、TURN、自前ホスト、話者別配信に適合   |
| TENをMode A実行基盤として利用     | **条件付きPoC採用** | 持続接続、割込み、Graph、Extension、監視機能が有効 |
| TENの設計思想だけをSonowaへ導入      | **即時採用**      | 低リスクで効果を得られる                     |
...
これは、現在のSonowaで発話ごとにRealtime Providerへ接続する方式よりも、ハンドシェイク回数を減らせる可能性が高い。

### 3.2.3 割込み制御

OpenAI Realtime Extensionは、ユーザー発話開始イベントを受けた際に、生成中のレスポンスを割込み扱いにし、以後の古い音声・テキストdeltaを破棄する。

Sonowaで必要としている以下の機能と一致する。
...
これは、Sonowaの現在のAI処理時間中心の監視を、Queue、Extension、Worker単位へ拡張する際に参考になる。

---

# 4. TENは本当にSonowaより高速か
...
TENの資料では、RTC経路について50～150msという目安を示しているが、これはTENとLiveKitまたはSonowaを同一条件で比較したベンチマークではない。
...
Sonowaは現在、LiveKitを自前ホストできる構成である。
...
Sonowaを社外販売、SaaS提供、会議基盤製品として展開する場合、この条件に抵触しないか法務確認が必要である。
...
flowchart LR
    Client[Web Client]
    LiveKit[LiveKit SFU]
    Gateway[Sonowa Realtime Gateway]
...
SonowaからTENへの依存を直接広げず、共通インターフェースを定義する。
...
これにより、TENが不採用になってもSonowaのアーキテクチャを戻す必要がない。
...
現行Sonowaは話者別Queueに上限を設定しているが、Queue満杯時に最古データを削除する処理がある。
...
```text
A系統
Sonowa Native Persistent Runtime
...
* [ ] TEN音声出力をSonowa Output Managerへ戻せる
* [ ] TEN EventをSonowa共通Eventへ変換できる
* [ ] TEN障害時にNativeへ戻せる
* [ ] TENなしでもSonowaを起動できる
...
> **WebRTC／LiveKitは維持する。**
> **Sonowa Native Runtimeを先に持続型Full-Duplexへ改善する。**
> **TEN FrameworkはMode A限定の比較PoCとして導入する。**
> **TENのGraph、Extension、割込み、Telemetryの設計思想はSonowaへ取り込む。**
