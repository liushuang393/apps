...
### Mistake 3b: Dual delivery interfaces across OM ↔ LiveKit

**Bad**: Output Manager talks `TransportAdapter` while tests/callers still use `deliver_*` / `OutputSinkTransportAdapter`

**Good**: Single seam — `build_default_output_manager(adapter)` with `publish_audio` / `send_data` only. See `.trellis/spec/backend/transport-adapter.md`.
