...
目的:
    ...
    ため、独自チャンク `SONO` にマーカー文字列を埋め込んでも再生に影響しない。
...
_DEFAULT_MARKER = "SONOWA-SYNTH"  # provenance marker payload（既定の埋め込み文字列）
_WATERMARK_CHUNK_ID = b"SONO"  # custom RIFF chunk id（4 バイト固定）
...
def read_watermark(wav: bytes) -> str | None:
    ...
    Args:
        wav: 解析対象の WAV バイト列。
    Returns:
        最初に見つかった SONO チャンクの marker 文字列。無ければ None。
    """
    try:
...
def is_watermarked(wav: bytes, marker: str = _DEFAULT_MARKER) -> bool:
    """WAV に指定 marker の SONO チャンクが在るか（解析不能なら False）。
...
def _build_chunk(marker: str) -> bytes:
    """marker から SONO チャンク（id + size + payload + 必要ならパディング）を作る。"""
