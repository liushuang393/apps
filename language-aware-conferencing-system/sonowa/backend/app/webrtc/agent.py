...
async def run_agent(
    room_id: str,
    *,
    identity: str = "sonowa-agent",
    display_name: str = "Sonowa Agent",
) -> None:
...
def main() -> None:
    """CLI/worker エントリ（room id は argv または SONOWA_AGENT_ROOM から取得）。"""
    import os
    import sys

    logging.basicConfig(level=logging.INFO)
    room_id = sys.argv[1] if len(sys.argv) > 1 else os.environ.get("SONOWA_AGENT_ROOM")
    if not room_id:
        raise SystemExit(
            "room id を argv[1] または SONOWA_AGENT_ROOM で指定してください"
        )
