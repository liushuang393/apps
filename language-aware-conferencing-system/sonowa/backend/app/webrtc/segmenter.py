...
# partial 無効を表す既定値（0 で従来どおり final のみを emit する）。
_DEFAULT_PARTIAL_MS = 0
# 既定（エネルギー）VAD のしきい値（16bit PCM の RMS、約 1.5%）。
_ENERGY_THRESHOLD = 500.0


class SegmentEvent(NamedTuple):
    """1 発話イベント（生 PCM と確定/暫定の別）。§P2 partial/final 事件協議。"""
...
class SpeechSegmenter:
    ...
    def __init__(
        ...
        *,
        ...
        partial_ms: int = _DEFAULT_PARTIAL_MS,
        ...
        # partial_ms>0 で暫定字幕を有効化（この累積発話フレーム毎に partial を出す）。
        self._partial_frames = (partial_ms // frame_ms) if partial_ms > 0 else 0
    ...
    def push(self, pcm: bytes) -> list[bytes]:
        ...
        後方互換のため partial は除外し final のみを返す。partial も要る場合は
        push_events() を使う（従来呼び出し側は無改変で動作する）。
        """
        return [e.pcm for e in self.push_events(pcm) if not e.is_partial]

    def push_events(self, pcm: bytes) -> list[SegmentEvent]:
        """PCM を取り込み、確定(final)/暫定(partial)イベント列を返す（§P2）。"""
    ...
    def _consume(self, frame: bytes, out: list[SegmentEvent]) -> None:
        """1 フレームを状態機械へ投入し、確定/暫定時に out へ追加する。"""
    ...
    def _maybe_emit_partial(self, out: list[SegmentEvent]) -> None:
        """partial 有効かつ間隔到達で暫定スナップショットを emit する（seg は保持）。"""
        if self._partial_frames <= 0:
            return
        if self._speech_frames < self._min_speech_frames:
            return
        if self._frames_since_partial >= self._partial_frames:
            out.append(SegmentEvent(bytes(self._seg), True))
            self._frames_since_partial = 0

    def _emit(self, out: list[SegmentEvent]) -> None:
        """現セグメントを確定する（最小発話長を満たす場合のみ emit）。"""
        if self._speech_frames >= self._min_speech_frames:
            out.append(SegmentEvent(bytes(self._seg), False))
        self._reset_segment()
