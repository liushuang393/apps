...
_AGENT_IDENTITY = "sonowa-agent"

_SAMPLE_RATE = 16000
_FRAME_SAMPLES = _SAMPLE_RATE * 20 // 1000  # 20ms
_JA_TEST_PHRASE = "今日は会議のテストです。翻訳が正しく動くか確認します。"
_TIMEOUT_SEC = 90.0
_API_BASE = os.getenv("SONOWA_API_BASE", "http://localhost:8090")
_LIVEKIT_URL = os.getenv("SONOWA_LIVEKIT_URL", "ws://localhost:7880")
...
os.environ.setdefault(
    "DATABASE_URL",
    "postgresql://sonowa:sonowa_secret_2024@localhost:5433/sonowa",
)
...
async def _wait_for_agent(room: rtc.Room, timeout: float = 30.0) -> None:
    ...
    while asyncio.get_event_loop().time() < deadline:
        ...
        if any(
            ...
            return
        await asyncio.sleep(0.5)
    raise TimeoutError("sonowa-agent が room に参加しませんでした")
