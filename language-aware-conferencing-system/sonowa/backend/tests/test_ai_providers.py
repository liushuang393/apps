...
実行方法:
    docker exec sonowa-backend-1 python -m pytest tests/test_ai_providers.py -v
    または
    docker exec sonowa-backend-1 python tests/test_ai_providers.py
"""
...
async def check_gemini_api() -> bool:  # pytest 収集対象外の接続診断
    ...
    try:
        ...
        response = client.models.generate_content(
            model=model,
            contents="Say 'Hello, Sonowa!' in one short sentence.",
        )
...
async def check_openai_api() -> bool:  # pytest 収集対象外の接続診断
    ...
    try:
        ...
        response = await client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "user", "content": "Say 'Hello, Sonowa!' in one short sentence."}
            ],
            max_tokens=50,
        )
