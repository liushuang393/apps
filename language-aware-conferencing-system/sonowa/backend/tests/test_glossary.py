...
def test_build_hint_do_not_translate() -> None:
    ...
    hint = build_prompt_hint(
        [GlossaryMatch("Sonowa", None, do_not_translate=True, priority=100)]
    )
    assert '"Sonowa"' in hint
...
def test_measure_hits_do_not_translate_preserved() -> None:
    ...
    matches = [GlossaryMatch("Sonowa", None, do_not_translate=True, priority=100)]
    assert measure_glossary_hits(matches, "Sonowa は便利です") == (1, 1)
