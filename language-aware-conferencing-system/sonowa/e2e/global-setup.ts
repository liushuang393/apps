...
function runPython(args: string[]): void {
  ...
  if (result.status !== 0) {
    throw new Error(
      `[sonowa/global-setup] reset_db ${args.join(' ')} 失敗\n` +
...
async function globalSetup(): Promise<void> {
  ...
  if (!process.env.E2E_DB_URL) {
    if (!requiresDb || process.env.E2E_ALLOW_NO_DB === '1') {
      console.log(
        `[sonowa/global-setup] E2E_DB_URL 未設定 → snapshot 省略 ` +
          `(seed_mode=${seedMode}, requires_db=${requiresDb})`,
      );
      return;
    }
    throw new Error(
      `[sonowa/global-setup] FAIL: seed_mode=${seedMode}, requires_db=true なのに ` +
  ...
  console.log(`[sonowa/global-setup] bootstrap baseline: ${SNAPSHOT_PATH}`);
