...
### 2. 参加端末の準備（Windows・1 台につき 1 回だけ）

参加端末は Windows（Chrome / Edge）を対象とする。ホストで設定キットを作り、各参加者に配る。

1. ホストで `scripts/make-client-kit.sh` を実行し、`output/sonowa-client-kit.zip` を作る（中身: `install-client.bat`・`setup-windows.ps1`・`ca.crt`・`はじめにお読みください.txt`。秘密鍵 `ca.key` は入らない）
2. zip を参加者へ渡す（共有フォルダ・USB・メール添付など）
3. 参加者は zip を展開し、`install-client.bat` をダブルクリック →「このアプリがデバイスに変更を加えることを許可しますか？」で「はい」→ 青い画面に `[OK] 完了` と出たら閉じる
4. Chrome / Edge をすべて閉じて開き直し、`https://<HOST_IP>` を開く。鍵マークが出て警告が無ければ完了

`install-client.bat` は社内 CA（`ca.crt`）を「信頼されたルート証明機関」（コンピューター全体）へ登録するだけで、Firewall などは変更しない。管理者権限が無い PC は、その PC の管理者に実行してもらう。登録はホストの IP が変わっても不要（サーバー証明書だけが自動で作り直される）。ホストの `certs/ca.crt` を消すと全端末で登録し直しになるため消さない。
...
**変更する（今のパスワードが分かる場合）**: ログイン後、メニューの「プロフィール」→「パスワード変更」で、現在のパスワードと新しいパスワード（8文字以上・2回）を入力して「パスワードを変更」。次回から新しいパスワードでログインする。

**忘れた場合**: ログイン画面の「パスワードをお忘れですか？」→ 登録したメールアドレスを入力 →「再設定へ進む」で、そのまま再設定画面へ進む（トークンは自動入力）。新しいパスワードを2回入力して「パスワードを更新」→ ログイン画面から新しいパスワードでログインする。未登録のアドレスは「このメールアドレスは登録されていません」と表示される。

管理者が代わりに再設定することもできる: `https://<HOST_IP>/admin` → 対象ユーザーの「編集」→「再設定リンクを発行」→ 表示されたリンクを本人へ渡す（1時間有効・1回のみ使用可）。

> メール送信が無いため、メールアドレスを知っていれば誰でもそのアカウントのパスワードを再設定できる（社内 LAN 前提の運用）。メール送信を導入したら `.env` に `PASSWORD_RESET_SELF_SERVICE=false` を設定し、再設定リンクをメールで届ける方式に切り替える。
...
| 昨日まで開けた URL が開けない | ホストの IP が変わっていないか（起動画面の URL を確認）。IP を固定する |
| パスワード再設定で「無効なリセットトークンです」 | 有効期限（1時間）切れ・使用済み・再発行で古くなった可能性。「パスワードをお忘れですか？」からやり直す |
