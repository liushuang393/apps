...
**忘れた場合**: 本番環境ではメールを送信しないため、管理者が再設定リンクを発行する。

1. 管理者が `https://<HOST_IP>/admin` を開き、対象ユーザーの「編集」→「再設定リンクを発行」を押す
2. 表示されたリンク（`https://<HOST_IP>/reset-password?token=...`）を本人へ渡す（1時間有効・1回のみ使用可。再発行すると古いリンクは無効）
3. 本人がリンクを開くとトークンが入力済みになっている。新しいパスワードを2回入力して「再設定」→ ログイン画面から新しいパスワードでログインする

ログイン画面の「パスワードをお忘れですか？」からは、本番では「管理者に再設定リンクの発行を依頼してください」と案内される（開発環境では画面にトークン付きリンクが出る）。
...
| パスワード再設定で「無効なリセットトークンです」 | リンクの有効期限（1時間）切れ・使用済み・再発行で古くなった可能性。管理者に再発行を依頼する |
