...
function runPython(args: string[]): void {
  ...
  if (result.status !== 0) {
    console.warn(
      `[lams/global-teardown] reset_db ${args.join(' ')} 警告\n` +
...
async function globalTeardown(): Promise<void> {
  ...
  if (!fs.existsSync(SNAPSHOT_PATH)) {
    console.warn(`[lams/global-teardown] snapshot 未生成: ${SNAPSHOT_PATH}`);
    return;
  }
  console.log(`[lams/global-teardown] restore from ${SNAPSHOT_PATH}`);
