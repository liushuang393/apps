...
> 再展開: `python testing-kit/bin/init-app.py --app lams --target e2e --reapply`
...
- 認証は JWT login のみ。auth bypass は禁止。`helpers/auth.ts` が `lams-auth` に zustand persist 形で注入する。
...
- `e2e/app-owned-runtime.json` の `data_identity` は共有 DB wipe 禁止のため
  `kind=file` + センチネル `e2e/.runtime/lams-shared-no-wipe.marker`（kit schema は file/jdbc/composite_file のみ）。
...
# DB リセット・seed
E2E_DB_URL=sqlite:////tmp/lams_e2e_test.db python scripts/reset_db.py
E2E_DB_URL=sqlite:////tmp/lams_e2e_test.db python scripts/seed_scenario.py --name basic
```
