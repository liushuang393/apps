...
export LAMS_FRONTEND="${LAMS_FRONTEND:-http://127.0.0.1:5273}"
...
api_ok=0
if curl -fsS --max-time 5 "${LAMS_API_BASE}/health" >/dev/null 2>&1; then
  api_ok=1
fi

# SQLite E2E DB（ローカルスタック）があれば admin 昇格に使う
SQLITE_DEFAULT="$ROOT/.scratch/lams_e2e.sqlite3"
if [[ -z "${LAMS_E2E_SQLITE_PATH:-}" && -f "$SQLITE_DEFAULT" ]]; then
  export LAMS_E2E_SQLITE_PATH="$SQLITE_DEFAULT"
fi

LIVEKIT_BLOCKERS=()
[[ "$has_openai" -eq 1 ]] || LIVEKIT_BLOCKERS+=("OPENAI_API_KEY missing")
[[ "$api_ok" -eq 1 ]] || LIVEKIT_BLOCKERS+=("API /health unreachable at ${LAMS_API_BASE}")
...

{
  ...
  echo "- LAMS_E2E_SQLITE_PATH set: $([[ -n "${LAMS_E2E_SQLITE_PATH:-}" ]] && echo yes || echo no)"
