...
- Postgres DB `lams_e2e`（URL に `e2e` を含め kit reset 可能）
...
**暫定**: `MockAIProvider`（`LAMS_E2E_MOCK_AI=1`／`ai_provider=mock`）  
...
kit は context 骨格のみ。中身は LAMS 固有。
