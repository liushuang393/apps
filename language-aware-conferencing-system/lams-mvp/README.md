...
- Docker & Docker Compose（推奨構成はこれだけで動く）
- ローカル起動時のみ: Python 3.10+ / Node.js 20+
...
**手で記入するのは APIキーだけ**（`secrets.json` は廃止済み）。それ以外は既定値・スクリプトで自動設定される。
...
| `HOST_IP` | LAN公開時のみ | `./scripts/set-host-ip.sh` が自動検出して `.env` を書き換え（後述） |
...
> `start-with-keys.sh` は APIキーを `.env` へ書き戻さない。
> 本番ではクラウドの環境変数 / Docker secrets を使用し、キーをディスクに置かないこと。

### 2-A. Docker 起動（推奨）

frontend / backend / postgres / redis / livekit / coturn を一括起動する。DB 接続・LiveKit 設定はすべて自動。

```bash
./start-with-keys.sh docker           # 通常起動
./start-with-keys.sh "docker build"   # 初回・依存更新時（ビルド付き）
```

`start-with-keys.sh` は `AI_PROVIDER` に必要な APIキーが無いと起動前に停止する（記入忘れ防止ガード）。

### 2-B. ローカル起動（backend 開発時）

DB / Redis / LiveKit は Docker、backend だけホストで動かす構成。

```bash
docker compose up -d postgres redis livekit    # 依存サービスのみ起動
./start-with-keys.sh                           # backend（DATABASE_URL 等は自動設定、port 8090）
cd frontend && npm install && npm run dev      # frontend（別ターミナル、port 5273）
```
...
社内の他PCからアクセスする場合。**基本はスクリプト1発で完了する**:

```bash
./scripts/set-host-ip.sh               # HOST_IP 自動検出 → .env 更新 → 関連コンテナ再起動
./scripts/set-host-ip.sh 192.168.x.y   # 自動検出に失敗した場合のみ IP を明示指定
```

DHCP で Windows の IP が変わって繋がらなくなったら、同じスクリプトをもう1回実行するだけで復旧する。
...
> - **マイク許可**のため、他PCの Chrome/Edge では `chrome://flags` の
>   「Insecure origins treated as secure」に `http://<IP>:5273` を登録する（自機で使う `localhost` は不要）。
