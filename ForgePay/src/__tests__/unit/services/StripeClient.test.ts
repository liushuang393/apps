...
describe('StripeClient', () => {
  ...
  describe('createCoupon', () => {
    ...
    beforeEach(() => {
      mockCouponsCreate.mockResolvedValue(mockCoupon);
    });
    ...
    it('should create a coupon with custom ID', async () => {
      await client.createCoupon({
        id: 'SUMMER2024',
        name: 'Summer Sale',
        percent_off: 15,
    ...
    it('should create a coupon with redeem by date', async () => {
      const redeemBy = Math.floor(Date.now() / 1000) + 30 * 24 * 60 * 60;
      await client.createCoupon({
        name: 'Expiring Offer',
        percent_off: 25,
        redeem_by: redeemBy,
    ...
    it('should log coupon creation', async () => {
      await client.createCoupon({ name: 'Test Coupon', percent_off: 10 });

      expect(logger.info).toHaveBeenCalledWith('Stripe coupon created', {
        couponId: 'coupon_test_123',
        name: 'Summer Sale',
      });
    });

    it('should handle errors when creating coupon', async () => {
      const error = new MockStripeInvalidRequestError('Invalid coupon parameters');
      mockCouponsCreate.mockRejectedValue(error);

      await expect(client.createCoupon({ name: 'Bad Coupon', percent_off: 150 })).rejects.toThrow('Invalid request: Invalid coupon parameters');
    });
  });
  ...
  describe('updateCoupon', () => {
    ...
    beforeEach(() => {
      mockCouponsUpdate.mockResolvedValue(mockUpdatedCoupon);
    });

    it('should update a coupon name', async () => {
      const result = await client.updateCoupon('coupon_test_123', {
        name: 'Updated Coupon Name',
      });

      expect(result).toEqual(mockUpdatedCoupon);
      expect(mockCouponsUpdate).toHaveBeenCalledWith('coupon_test_123', {
        name: 'Updated Coupon Name',
      });
    });

    it('should update coupon metadata', async () => {
      await client.updateCoupon('coupon_test_123', {
    ...
    it('should log coupon update', async () => {
      await client.updateCoupon('coupon_test_123', { name: 'New Name' });

      expect(logger.info).toHaveBeenCalledWith('Stripe coupon updated', {
        couponId: 'coupon_test_123',
      });
    });

    it('should handle errors when updating coupon', async () => {
      const error = new MockStripeInvalidRequestError('Coupon not found');
      mockCouponsUpdate.mockRejectedValue(error);

      await expect(client.updateCoupon('invalid_coupon', { name: 'Test' })).rejects.toThrow('Invalid request: Coupon not found');
    });
  });

  describe('deleteCoupon', () => {
    const mockDeletedCoupon = {
      id: 'coupon_test_123',
      deleted: true,
    };

    beforeEach(() => {
      mockCouponsDel.mockResolvedValue(mockDeletedCoupon);
    });

    it('should delete a coupon', async () => {
      const result = await client.deleteCoupon('coupon_test_123');

      expect(result).toEqual(mockDeletedCoupon);
      expect(mockCouponsDel).toHaveBeenCalledWith('coupon_test_123');
    });

    it('should log coupon deletion', async () => {
      await client.deleteCoupon('coupon_test_123');

      expect(logger.info).toHaveBeenCalledWith('Stripe coupon deleted', {
        couponId: 'coupon_test_123',
      });
    });

    it('should handle errors when deleting coupon', async () => {
      const error = new MockStripeInvalidRequestError('Coupon not found');
      mockCouponsDel.mockRejectedValue(error);

      await expect(client.deleteCoupon('invalid_coupon')).rejects.toThrow('Invalid request: Coupon not found');
    });
  });
  ...
  describe('getCoupon', () => {
    ...
    beforeEach(() => {
      mockCouponsRetrieve.mockResolvedValue(mockCoupon);
    });

    it('should retrieve a coupon', async () => {
      const result = await client.getCoupon('coupon_test_123');

      expect(result).toEqual(mockCoupon);
      expect(mockCouponsRetrieve).toHaveBeenCalledWith('coupon_test_123');
    });

    it('should handle errors when retrieving coupon', async () => {
      const error = new MockStripeInvalidRequestError('Coupon not found');
      mockCouponsRetrieve.mockRejectedValue(error);

      await expect(client.getCoupon('invalid_coupon')).rejects.toThrow('Invalid request: Coupon not found');
      expect(logger.error).toHaveBeenCalled();
    });
  });
