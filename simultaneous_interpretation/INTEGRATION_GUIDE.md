...
| あなたのアプリ | `3002+` | 自由に設定 |
| PostgreSQL | `5442` (`FORGE_PAY_DB_PORT`) | ForgePay 用 DB |
| Redis | `6381` (`FORGE_PAY_REDIS_PORT`) | ForgePay 用キャッシュ |
...
# 11. Python 統合（infrastructure/payment_client 経由）

AgentFlow プロジェクト本体 (Python / FastAPI) からは、Node.js の forge_pay HTTP API を `infrastructure/payment_client/` の Python SDK 経由で呼び出します。auth と同じく「最小設定で組み込める」設計です。

## 11.1 5 分で課金開始（Cursor 風フロー）

```bash
# 1) forge_pay 起動
cd apps/common_services/forge_pay
npm ci && npm run migrate:up && npm run dev

# 2) developer 登録（テスト用 API キー取得）
curl -X POST http://localhost:3000/api/v1/onboarding/register \
  -H "Content-Type: application/json" \
  -d '{"email":"dev@example.com","testMode":true}'
# レスポンスの apiKey.key をメモ

# 3) Python SDK インストール
cd /home/liush/projects/serverlessAIAgents
conda run -n agentflow pip install -e infrastructure/payment_client

# 4) preset を適用（Pro プラン: 月 500 回まで）
curl -X POST http://localhost:3000/api/v1/presets/apply \
  -H "X-API-Key: <取得したapiKey>" \
  -H "Content-Type: application/json" \
  -d '{"preset":"pro","app":"dge","event_type":"decision.request"}'

# 5) Python アプリから quota チェック
conda run -n agentflow python -c "
import asyncio
from infrastructure.payment_client import build_payment_client

async def main():
    client = build_payment_client()
    quota = await client.check_quota(tenant_id='tenant-1', event_type='decision.request')
    print(f'allowed={quota.allowed}, remaining={quota.remaining}/{quota.limit}')
    await client.aclose()
...
## 11.2 FastAPI への組み込み（最小 2 行）

```python
# apps/your_app/main.py
from fastapi import FastAPI, Depends
from infrastructure.payment_client import require_quota

app = FastAPI()

@app.post("/api/decision")
async def decision(_quota: None = Depends(require_quota("decision.request"))):
    # 既存のビジネスロジック
    return {"result": "ok"}
```

これで `/api/decision` が呼ばれるたびに自動で quota チェックが入ります（preset で設定した制限を超えると 402 を返却）。
...
| `free` | $0 | 50 | 個人お試し |
| `pro` | $20 | 500 | 個人プロ |
| `business` | $40/user | 5,000 | チーム / 中小企業 |
| `enterprise` | お問合せ | 無制限 | エンタープライズ |

`GET /api/v1/presets` で実際の一覧と feature 詳細を取得できます。各 app は `app_config.json` の `contracts.payment.preset` で指定するだけで完了です。

## 11.4 Platform-Managed Stripe モード（開発者は Stripe アカウント不要）

`developers.stripe_mode = 'platform'` に設定すると、forge_pay プラットフォーム共有の Stripe アカウントで集約決済できます。開発者は Stripe Secret を自前で用意せずに済むため、サインアップ → プラン選択 → 即課金開始という Cursor 風の体験が実現します。

切替方法（onboarding API 経由）:

```bash
# 既存の onboarding に PATCH 経由で stripe_mode='platform' を設定
# （詳細は src/routes/onboarding.ts を参照）
```

詳細な手順と運用上の注意は [`docs/DEPLOY.md`](docs/DEPLOY.md) と [`docs/PYTHON_QUICKSTART.md`](docs/PYTHON_QUICKSTART.md) を参照してください。
