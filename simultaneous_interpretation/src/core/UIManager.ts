...
export class UIManager {
    ...
    updateConnectionStatus(status: ConnectionStatus): void {
        ...
        console.log('[UIManager] 接続状態更新:', status);
    }
    ...
    updateStatus(type: string, text: string): void {
        console.log(`[UIManager] ${type}: ${text}`);
    }
    ...
    addTranscript(type: TranscriptType, text: string, transcriptId?: number): void {
        ...
        const transcriptElement = type === 'input' 
        ...
        if (transcriptId && type === 'output') {
            const existingMessage = transcriptElement.querySelector(`[data-transcript-id="${transcriptId}"]`);
            if (existingMessage) {
                console.log('[UIManager] 重複トランスクリプトをスキップ:', transcriptId);
                return;
            }
        }
        ...
        console.log('[UIManager] トランスクリプト追加:', { type, text: text.substring(0, 50), transcriptId });
    }
    ...
    clearTranscript(type: TranscriptType = 'both'): void {
        console.log('[UIManager] トランスクリプトクリア:', type);
    ...
    notify(options: NotificationOptions): void {
        ...
        if (!notification || !titleEl || !messageEl) {
            ...
            console.log(`[UIManager] 通知: ${options.title} - ${options.message}`);
            return;
        }
        ...
        console.log('[UIManager] 通知表示:', options);
    }
