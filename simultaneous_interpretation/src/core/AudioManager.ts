...
export class AudioManager {
    ...
    setInputAudioOutputEnabled(enabled: boolean): void {
        ...
        if (this.inputGainNode) {
            ...
            defaultLogger.info('[AudioManager] 入力音声出力:', enabled ? 'ON' : 'OFF');
        }
    }
    ...
    async startMicrophoneCapture(constraints: AudioConstraints): Promise<void> {
        defaultLogger.info('[AudioManager] マイクキャプチャを開始...');
        ...
        defaultLogger.info('[AudioManager] マイクアクセス要求中...', audioConstraints);

        try {
            this.mediaStream = await navigator.mediaDevices.getUserMedia(audioConstraints);
            defaultLogger.info('[AudioManager] マイクアクセス取得成功');
        } catch (error: unknown) {
            defaultLogger.error('[AudioManager] マイクアクセス取得失敗:', error);
    ...
    async startBrowserSystemAudioCapture(): Promise<void> {
        defaultLogger.info('[AudioManager] ブラウザ環境でシステム音声をキャプチャ...');
            ...
            if (audioTracks.length > 0) {
                ...
                if (audioTrack) {
                    audioTrack.addEventListener('ended', () => {
                        defaultLogger.error('[AudioManager] 音声トラックが停止しました');
                    });
                }
            }

            defaultLogger.info('[AudioManager] システム音声キャプチャ成功');
        } catch (error) {
            defaultLogger.error('[AudioManager] システム音声キャプチャ失敗:', error);
    ...
    async startTabAudioCapture(): Promise<void> {
        defaultLogger.info('[AudioManager] タブ音声キャプチャを開始...');
        ...
        try {
            ...
            defaultLogger.info('[AudioManager] タブ音声キャプチャ成功');
        } catch (error) {
            defaultLogger.error('[AudioManager] タブ音声キャプチャ失敗:', error);
            throw error;
        }
    }
    ...
    async startElectronSystemAudioCapture(sourceId?: string): Promise<void> {
        defaultLogger.info('[AudioManager] Electron環境でシステム音声をキャプチャ...');
        ...
        try {
            ...
            if (audioTracks.length === 0) {
                ...
                await new Promise<void>((resolve) => {
                    const checkInterval = setInterval(() => {
                        ...
                        if (tracks.length > 0) {
                            ...
                            defaultLogger.info('[AudioManager] 音声トラックが追加されました');
                            resolve();
                        }
                    }, 100);
                });
                ...
                this.mediaStream = stream;
                defaultLogger.info('[AudioManager] Electronシステム音声キャプチャ成功');
            }
        } catch (error) {
            defaultLogger.error('[AudioManager] Electronシステム音声キャプチャ失敗:', error);
            throw error;
        }
    }
    ...
    async setupAudioProcessing(): Promise<void> {
        ...
        defaultLogger.info('[AudioManager] 音声処理をセットアップ中...');
        ...
        if (this.audioContext.state === 'suspended') {
            defaultLogger.info('[AudioManager] AudioContextがサスペンド状態です。再開します...');
            await this.audioContext.resume();
            defaultLogger.info('[AudioManager] AudioContext再開完了:', this.audioContext.state);
        }
        ...
        defaultLogger.info('[AudioManager] 音声処理を開始...');
    ...
    async stopRecording(): Promise<void> {
        defaultLogger.info('[AudioManager] 停止処理開始');
        ...
        if (this.audioSource) {
            ...
            defaultLogger.info('[AudioManager] MediaStreamSource をクリーンアップしました');
        }
        ...
        if (this.inputGainNode) {
            ...
            defaultLogger.info('[AudioManager] GainNode をクリーンアップしました');
        }
        ...
        if (this.workletNode) {
            ...
            defaultLogger.info('[AudioManager] AudioWorkletNode をクリーンアップしました');
        }
        ...
        if (this.processor) {
            ...
            this.processor = null;
            defaultLogger.info('[AudioManager] ScriptProcessorNode をクリーンアップしました');
        }
        ...
        defaultLogger.info('[AudioManager] 停止処理完了');
    }
    ...
    processAudioQueue(): void {
        if (this.audioQueue.length === 0) {
            defaultLogger.info('[AudioManager] 音声キューが空です');
            return;
        }
        ...
        defaultLogger.info('[AudioManager] 音声を再生キューに追加:', {
    ...
    private playNextInQueue(): void {
        if (this.playbackQueue.length === 0) {
            defaultLogger.info('[AudioManager] 再生キューが空です');
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (this.isPlayingAudio) {
                    defaultLogger.info('[AudioManager] 音声再生中のため、入力音声をミュート');
                }
                ...
                if (!this.outputAudioContext) {
                    ...
                    defaultLogger.info('[AudioManager] 出力専用AudioContextを作成しました');
                }

                // AudioContextがsuspended状態の場合はresume
                if (this.outputAudioContext.state === 'suspended') {
                    await this.outputAudioContext.resume();
                    defaultLogger.info('[AudioManager] AudioContextをresumeしました');
                }
                ...
                source.onended = () => {
                    ...
                    defaultLogger.info('[AudioManager] 音声再生完了');
                ...
                defaultLogger.info('[AudioManager] 音声再生開始');
            } catch (error) {
                defaultLogger.error('[AudioManager] 音声再生エラー:', error);
                this.isPlayingAudio = false;
                reject(error);
            }
        });
    ...
    clearAudioQueue(): void {
        ...
        this.playbackQueue = [];
        this.isPlayingFromQueue = false;
        defaultLogger.info('[AudioManager] 音声キューをクリアしました');
    }
}
