...
const realtimeWebSocket_1 = require("./realtimeWebSocket");
...
function createMainWindow() {
    ...
    if (process.platform === 'darwin') {
        ...
        console.info(`[Main] macOS Media Access - Microphone: ${micStatus}, Screen: ${screenStatus}`);
    ...
    const htmlPath = isDev
        ...
        : path.join(electron_1.app.getAppPath(), 'teams-realtime-translator.html');
...
function registerIPCHandlers() {
    ...
    // 常に最前面に表示を切り替え
    electron_1.ipcMain.on('toggle-always-on-top', () => {
        if (mainWindow) {
    ...
    // 音声ソースを取得
    electron_1.ipcMain.handle('get-audio-sources', async (_event, types) => {
        return await audioCapture_1.ElectronAudioCapture.getAudioSources(types);
    });
    // 会議アプリを検出
    electron_1.ipcMain.handle('detect-meeting-apps', async () => {
        return await audioCapture_1.ElectronAudioCapture.detectMeetingApps();
    });
    // 音声トラックの有無を確認（レンダラープロセスから呼ばれる）
    electron_1.ipcMain.handle('check-audio-track', async (_event, sourceId) => {
    ...
    // 音声ソース ID を検証
    electron_1.ipcMain.handle('validate-source-id', async (_event, sourceId) => {
        return await audioCapture_1.ElectronAudioCapture.validateSourceId(sourceId);
    });
    // 環境変数からAPIキーを取得
    electron_1.ipcMain.handle('get-env-api-key', async () => {
        // 複数の環境変数名をチェック
    ...
    electron_1.ipcMain.handle('get-env-config', async () => {
        ...
        // エラーがある場合は例外を投げる
        if (errors.length > 0) {
            const errorMessage = `設定エラー: 必須の環境変数が設定されていません\n` +
                `${errors.join('\n')}\n\n` +
    ...
    // ✅ 会話データベース IPC ハンドラー
    // セッション開始
    electron_1.ipcMain.handle('conversation:start-session', (_event, sourceLanguage, targetLanguage) => {
        if (!conversationDB) {
    ...
    // セッション終了
    electron_1.ipcMain.handle('conversation:end-session', () => {
        if (!conversationDB) {
            throw new Error('Conversation database not initialized');
        }
    ...
    // ターン追加
    electron_1.ipcMain.handle('conversation:add-turn', (_event, turn) => {
        if (!conversationDB) {
            throw new Error('Conversation database not initialized');
        }
    ...
    // 最近のターンを取得
    electron_1.ipcMain.handle('conversation:get-recent-turns', (_event, count = 10, sessionId) => {
        if (!conversationDB) {
            throw new Error('Conversation database not initialized');
        }
    ...
    // API用のコンテキストを取得
    electron_1.ipcMain.handle('conversation:get-context-for-api', (_event, count = 10, sessionId) => {
        if (!conversationDB) {
            throw new Error('Conversation database not initialized');
        }
    ...
    // 統計情報を取得
    electron_1.ipcMain.handle('conversation:get-stats', () => {
        if (!conversationDB) {
            throw new Error('Conversation database not initialized');
        }
    ...
    // すべてのセッションを取得
    electron_1.ipcMain.handle('conversation:get-all-sessions', (_event, limit = 100) => {
        if (!conversationDB) {
            throw new Error('Conversation database not initialized');
        }
    ...
    // セッションのすべてのターンを取得
    electron_1.ipcMain.handle('conversation:get-session-turns', (_event, sessionId) => {
        if (!conversationDB) {
            throw new Error('Conversation database not initialized');
        }
    ...
    // 古いセッションを削除
    electron_1.ipcMain.handle('conversation:cleanup-old-sessions', (_event, daysToKeep = 30) => {
        if (!conversationDB) {
            throw new Error('Conversation database not initialized');
        }
 ...
 */
const chromiumCommandLine = electron_1.app?.commandLine;
...
async function requestMediaPermissions() {
    ...
    if (platform === 'darwin') {
        ...
        try {
            const micStatus = electron_1.systemPreferences.getMediaAccessStatus('microphone');
            const cameraStatus = electron_1.systemPreferences.getMediaAccessStatus('camera');
            const screenStatus = electron_1.systemPreferences.getMediaAccessStatus('screen');
            console.info(`[Main] macOS permissions - Microphone: ${micStatus}, Camera: ${cameraStatus}, Screen: ${screenStatus}`);
            if (micStatus !== 'granted') {
...
electron_1.app.whenReady().then(async () => {
    ...
    if ((0, ConversationDatabase_1.isElectronEnvironment)()) {
        try {
            ...
            if (customDbPath) {
                console.info('[Main] Using custom database path from CONVERSATION_DB_PATH:', customDbPath);
            }
            conversationDB = new ConversationDatabase_1.ConversationDatabase(customDbPath);
    ...
    registerIPCHandlers();
    (0, realtimeWebSocket_1.initializeRealtimeWebSocket)();
...
electron_1.app.on('will-quit', () => {
    ...
    // WebSocket接続をクリーンアップ
    (0, realtimeWebSocket_1.cleanupRealtimeWebSocket)();
    // ✅ 会話データベースを閉じる
    if (conversationDB) {
