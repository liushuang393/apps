...
describe('ResponseQueue', () => {
    ...
    describe('enqueue', () => {
        ...
        it('should reject concurrent requests', async () => {
            ...
            await expect(queue.enqueue(request2)).rejects.toThrow(
                'Previous response is still in progress'
            );
    ...
    describe('handleResponseCreated', () => {
        it('should handle response.created event', () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(
                expect.stringContaining('[ResponseQueue] レスポンス作成:'),
                'test-id'
            );
    ...
    describe('handleError', () => {
        ...
        it('should handle conversation_already_has_active_response error via code flag', async () => {
            ...
            const error = new Error(
                'Conversation already has an active response in progress: resp_test'
            );
