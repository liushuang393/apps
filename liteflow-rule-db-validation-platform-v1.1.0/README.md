...
**合格基準**: `ローカル検証 PASS: tests=78 failures=0 errors=0`
...
> Grafana は初回ログイン後にパスワード変更を促されますが、**Skip** で進めて構いません。
...
| `approvals.html` | 変更申請フォーム、申請一覧、**承認／却下**ボタン |
| `audit.html` | 監査ログ |
...
3. `scripts\local-verify.cmd`（約30秒。78テスト全部）
